...
class GUI(QMainWindow):
    ...
    def init_s_vid_frame(self):
        ...
        # self.s_vid.start()
        example_image = 'support/examples/example1.png'
    ...
    def process_complete_messagebox(self):
        ...
        """
        self.s_progressBar.setValue(100)
        if self.s_video_output_path == "":
            try:
                self.player.setMedia(QMediaContent(QUrl.fromLocalFile('support/examples/Output.avi')))
        ...
        if self.c_video_output_path == "":
            try:
                ...
                self.coronal_player.setMedia(
                    QMediaContent(QUrl.fromLocalFile('support/examples/coronal_output.avi')))
    ...
    def set_plot_dropdown(self, text):
        ...
        if text == "Filtered Cadence Scatter Plot":
            ...
            self.RAYTplot = self.display.create_graph("Right Ankle Trajectory", "Frame number", "Y Position (px)",
                                                      ...
                                                      True)
            self.RAYTplot.savefig("plots/RightAnkleTrajectory.png")
            ...
            # bf.create_custom_scatterplot(list(range(len(self.display.data.data_files))), self.display.num_distances, 10000, "RKST")
            ...
            # bf.create_custom_scatterplot(list(range(len(self.display.data.data_files))), self.display.num_angles,100, "RKST")
            if not self.plot_index:
                self.plot_index = 4
            else:
                self.plot_index += 1
