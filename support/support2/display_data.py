...
class DisplayData:
    ...
    def __init__(self, data):
        ...
        self.gui = None
        self.angle_graph_completed = None
        self.distance_graph_completed = None
        # define distances
        self.distances = []
        ...
        if args['video']:
            ...
            try:
                self.video_path = 'support/examples/example_sagittal.avi'
            except Exception as e:
                print("Do not remove the example images/videos! Please replace them to run program")
                sys.exit()
        ...
        try:
            ...
            raise e
        self.points = []
    def get_gradient_angle(self, pt1, pt2, pt3, pt4):
        try:
            g_1 = (pt2[1] - pt1[1]) / (pt2[0] - pt1[0])
        except ZeroDivisionError:
            g_1 = 9999
        try:
            g_2 = (pt4[1] - pt3[1]) / (pt4[0] - pt3[0])
        except ZeroDivisionError:
            g_2 = 9999
        theta1 = np.arctan(g_1)
        theta2 = np.arctan(g_2)
        angle = abs(np.degrees(theta1 - theta2))
        if angle > 90:
            angle = 180 - angle
        return angle
    ...
    def compare_validation(self):
        self.data.read_motion_capture('S03_mocap_walking_009.csv')
        mocap = self.data.mocap_dist
        distances = []
        ...
        distances = [x * 2.4028 for x in distances]
        rmse = []
        '''
        for idx, dist in enumerate(distances):
            print(dist, mocap[idx])
            rmse.append(self.rmse(dist, mocap[idx]))
            print(rmse)
        '''
        ...
        angles = []
        mocap_angles = []
        for i in range(len(self.data.L_toe)):
            a = self.get_gradient_angle(self.data.LKnee[i], self.data.LGtroch[i], self.data.RKnee[i], self.data.RGtroch[i])
            mocap_angles.append(a)


        for idx, path in enumerate(self.data.input_files):
            angle = self.get_gradient_angle(self.fp("LKnee", idx), self.fp("LHip", idx), self.fp("RKnee", idx),
                                            self.fp("RHip", idx))
            angles.append(angle)
        ...
        mocap_x = []
        count = 0
        for i in range(len(mocap)):
            mocap_x.append(count)
            count += 8.001634655
            print(i, "mocap")
        print(count, "mocap")
        # op_x = list(range(len(distances)))
        count = 220
        op_x = []
        ...
        if length % 2 == 0:
            length -= 1
            del op_x[-1]
        b, a = scipy.signal.butter(4, .0099, analog=False)
        '''
        # angles = scipy.signal.filtfilt(b, a, angles)
        # distances = scipy.signal.filtfilt(b, a, angles)
        '''
        plt.figure()
        plt.plot(mocap_x, mocap, linewidth=2, linestyle="-", c="b")
        ax2 = plt.twinx()
        ax2.plot(op_x, distances, linewidth=2, linestyle="-", c="r")
        plt.title('Mocap vs OpenPose')
        # axes = plt.gca()
        #ymin = 0
        #ymax = 5000
        #axes.set_ylim([ymin, ymax])
        plt.xlabel('frame number')
        plt.set_ylabel('millmitres mocap')
        ax2.set_ylabel('millmitres op')
        '''
        type = 2
        if type == 2:
            plt.figure()
            plt.plot(mocap_x, mocap_angles, linewidth=2, linestyle="-", c="b", label="Mocap")
            plt.plot(op_x, angles, linewidth=2, linestyle="-", c="r", label = "OP")
        ...
        if type == 1:
            ...
            color = 'tab:red'
            ax1.set_xlabel('time')
            ax1.set_ylabel('mocap mm', color=color)
            ax1.plot(mocap_x, mocap, color=color)
            ...
            color = 'tab:blue'
            ax2.set_ylabel('OP mm', color=color)  # we already handled the x-label with ax1
            ax2.plot(op_x, distances, color=color)
            ax2.tick_params(axis='y', labelcolor=color)

            fig.tight_layout()  # otherwise the right y-label is slightly clipped
        if type == 1:
            plt.figure()
            plt.plot(mocap_x, mocap, linewidth=2, linestyle="-", c="b", label = "Mocap")
            plt.plot(op_x, distances, linewidth=2, linestyle="-", c="r", label = "OP")
            plt.legend(loc="upper left")
            plt.title('Mocap vs OpenPose')
            # axes = plt.gca()
            # ymin = 0
            ...
            print('test', plt.gca().get_lines()[0].get_xydata())
            print(len(plt.gca().get_lines()[0].get_xydata()))

        # plt.gca().set_aspect('equal', adjustable='box')
        plt.show()

        # plt.savefig("plots/OpMocap.png")
        p = input()
        sys.exit()
    ...
    def select_points(self):
        ...
        while True:
            ...
            # TODO change resize here
            # self.image = ResizeWithAspectRatio(self.image, width=800)
    ...
    def distance_overlay(self):
        ...
        if not self.distance_graph_completed:
            self.distance_graph_completed = True
    ...
    def angle_overlay(self):
        ...
        if not self.angle_graph_completed:
            self.angle_graph_completed = True
