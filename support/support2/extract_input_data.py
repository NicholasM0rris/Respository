...
try:
    ...
    if not os.path.exists('coronal_input_data'):
        os.makedirs('coronal_input_data')
    if not os.path.exists('coronal_input_images'):
        os.makedirs('coronal_input_images')
...
class ExtractData:
    ...
    def read_motion_capture(self, filename):
        import csv
        import numpy as np
        time = []
        body_parts = {}
        with open(filename) as csv_file:
            ...
            for row in csv_reader:
                # Obtain part labels and their column index
                if line_count == 3:
        ...
        # coords in millimetre
        l_ankle_lat = [[float(x[0]), float(x[1])] for x in body_parts["MOAS:L_ank_lat"]]
        r_ankle_lat = [[float(x[0]), float(x[1])] for x in body_parts["MOAS:R_ank_lat"]]
        self.L_toe = [[float(x[0]), float(x[1])] for x in body_parts["MOAS:L_Toe"]]
        self.R_toe = [[float(x[0]), float(x[1])] for x in body_parts["MOAS:R_Toe"]]
        self.LGtroch = [[float(x[0]), float(x[1])] for x in body_parts["MOAS:L_Gtroch"]]
        self.RGtroch = []

        for x in body_parts["MOAS:R_Gtroch"]:
            try:
                self.RGtroch.append([float(x[0]), float(x[1])])
                a = [float(x[0]), float(x[1])]
            except Exception as e:
                self.RGtroch.append(a)
        print(self.RGtroch)
        #print(self.LGtroch)


        self.LKnee = [[float(x[0]), float(x[1])] for x in body_parts["MOAS:L_knee_lat"]]
        self.RKnee = [[float(x[0]), float(x[1])] for x in body_parts["MOAS:R_knee_lat"]]
        print('1', np.array(self.L_toe[0]) - np.array(self.R_toe[0]))
        print('2', np.array(self.L_toe[0]))
        self.mocap_dist = []
        for i in range(len(self.L_toe)):
            self.mocap_dist.append(np.linalg.norm(np.array(self.L_toe[i]) - np.array(self.R_toe[i])))
    ...
    def interpolate_all_keypoints(self):
        """
        Interpolate between points to estimate the locations of the missing key points not found by Open Pose
        """
        # print(self.key_points)
